...
namespace LayoutManager.Tools {
	...
	public class ComponentEditingTools : System.ComponentModel.Component, ILayoutModuleSetup {
		...
		/// </summary>
		PropertiesDialogMapEntry[]	propertiesDialogMap = 
		...
		private bool canTestComponent(ModelComponent component) {
            IModelComponentConnectToControl connectableComponent = component as IModelComponentConnectToControl;

			if(connectableComponent != null && connectableComponent.FullyConnected) {
			...
			try {
				if(LayoutController.Instance.BeginDesignTimeActivation()) {
                    ...
                    if(testDialog == null) {
                        ...
                        else if(testObject is ControlConnectionPoint) {
                            ControlConnectionPoint controlCp = (ControlConnectionPoint)testObject;
                        ...
                        else if(testObject is ControlConnectionPointReference) {
                            ControlConnectionPointReference controlCpRef = (ControlConnectionPointReference)testObject;
		...
		public class SignalBindSignalMenuItem : MenuItem {
			LayoutSignalComponent				signal;
			IModelComponentReceiverDialog	dialog;
		...
		class ShiftComponentsMenuEntryBase : MenuItem {
			...
			OperationType		operation = OperationType.ShiftAreaComponents;
					...
					foreach(LayoutModelSpotComponentCollection spot in minorAxis.Values) {
						...
						if(existingSpot  != null) {
							...
							foreach(LayoutCommand command in shiftCommand) {
								LayoutMoveModelSpotCommand moveSpotCommand = command as LayoutMoveModelSpotCommand;

								if(moveSpotCommand != null && moveSpotCommand.Spot == existingSpot) {
									deleteIt = false;
									break;
								}
							}
			...
			protected SortedList<int, SortedList<int, LayoutModelSpotComponentCollection>>GetSelectionSpotList(LayoutSelection selection, ChangeCoordinate majorAxisIs, CompareValue.Order sortOrder) {
				...
				foreach(ModelComponent component in selection) {
					if(component.Spot.Area == HitTestResult.Area) {
						...
						SortedList<int, LayoutModelSpotComponentCollection> minorAxis;

						if(!majorAxis.TryGetValue(majorAxisIndex, out minorAxis)) {
			...
			protected void FillHorizontalGap(IDictionary<int, int> boundries, LayoutCompoundCommand shiftCommand) {
				...
				foreach(KeyValuePair<int, int> boundryPair in boundries) {
					...
					LayoutModelSpotComponentCollection fixedSpot;
					...
					if(area.Grid.TryGetValue(new Point(movedSpot.Location.X, fixedSpotY), out fixedSpot)) {
						...
						if(movedTrack != null && fixedTrack != null) {
							if(movedTrack.HasConnectionPoint(LayoutComponentConnectionPoint.B) && fixedTrack.HasConnectionPoint(LayoutComponentConnectionPoint.T)) {
								...
								for(int i = 0; i < count; i++) {
									...
									LayoutBridgeComponent bridge = movedTrack.TrackBackground as LayoutBridgeComponent;
									...
									LayoutTunnelComponent tunnel = movedTrack.TrackBackground as LayoutTunnelComponent;
			...
			protected void FillVerticalGap(IDictionary<int, int> boundries, LayoutCompoundCommand shiftCommand) {
				...
				foreach(KeyValuePair<int, int> boundryPair in boundries) {
					...
					if(area.Grid.TryGetValue(new Point(fixedSpotX, movedSpot.Location.Y), out fixedSpot)) {
			...
			private IDictionary<int, int> FindBoundries(LayoutSelection selection) {
				...
				foreach(ModelComponent component in selection) {
					...
					if(!boundries.TryGetValue(i, out boundryValue))
						boundries[i] = v;
			...
			public void DoOperation(int moveBy, bool fillGap) {
   			...
   			protected override void OnClick(EventArgs e) {
				if(changeWhat == ChangeCoordinate.Prompt) {
					...
					d.Text = (Operation == OperationType.MoveSelection) ? "Move Selection" : "Shift Components";
		...
		class ShowControlModuleLocationMenuItem : MenuItem {
			LayoutControlModuleLocationComponent	component;
