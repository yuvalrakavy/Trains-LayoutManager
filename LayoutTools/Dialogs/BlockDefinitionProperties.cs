...
namespace LayoutManager.Tools.Dialogs
{
	...
	public class BlockInfoProperties : Form, ILayoutComponentPropertiesDialog, CommonUI.Controls.IPolicyListCustomizer
	{
		...
		private void buttonOK_Click(object sender, System.EventArgs e) {
				...
				if(checkBoxSuggestAsDestination.Checked || checkBoxSuggestForLocomotivePlacement.Checked) {
					...
					if(nextBlockNumber == -1) {
						foreach(LayoutBlockDefinitionComponent otherBlockInfo in LayoutModel.Components<LayoutBlockDefinitionComponent>(LayoutPhase.All)) {
							...
							if(otherBlockInfo.Name.StartsWith("Block #") && int.TryParse(otherBlockInfo.Name.Substring(7), out blockNumber)) {
			...
			info.SuggestForPlacement       = checkBoxSuggestForLocomotivePlacement.Checked;
			info.SuggestForDestination     = checkBoxSuggestAsDestination.Checked;
			info.SuggestForProgramming     = checkBoxSuggestForProgramming.Checked;
			info.TrainLengthLimit          = trainLengthDiagram.Length;
			info.IsOccupancyDetectionBlock = checkBoxOccupancyDetectionBlock.Checked;
