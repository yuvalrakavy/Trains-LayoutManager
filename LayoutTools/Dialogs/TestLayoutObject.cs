...
namespace LayoutManager.Tools.Dialogs {
    partial class TestLayoutObject : Form {
		...
		protected int State {
			...
			set {
				...
				if(state >= 0) {
					...
					EventManager.AsyncEvent(new LayoutEvent<ControlConnectionPointReference, int>("change-track-component-state-command", connectionPointRef, stateToSend).SetCommandStation(connectionPointRef));
		...
		private void panelIllustration_Paint(object sender, PaintEventArgs e) {
			...
			if(component is LayoutTurnoutTrackComponent) {
				LayoutTurnoutTrackComponent turnout = (LayoutTurnoutTrackComponent)component;
			...
			else if(component is LayoutDoubleSlipTrackComponent) {
				LayoutDoubleSlipTrackComponent doubleSlip = (LayoutDoubleSlipTrackComponent)component;
