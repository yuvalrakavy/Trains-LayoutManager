	...
	public class ComponentOperationTools : System.ComponentModel.Component, ILayoutModuleSetup {
		...
		private void commandStationEmergencyStopNotification(LayoutEvent e) {
			...
			if(activeNotification != null) {
				...
				activeNotification.Owner = frameWindow as Form;
		...
		public ComponentOperationTools(System.ComponentModel.IContainer container) {
			/// <summary>
		...
		private void addPowerConnectionEntries(LayoutBlockDefinitionComponent blockDefinition, Menu m) {
			...
			foreach(var power in blockDefinition.PowerConnector.Inlet.ConnectedOutlet.ObtainablePowers) {
				if(power.PowerOriginComponentId != blockDefinition.Power.PowerOriginComponentId || power.Type != blockDefinition.Power.Type) {
					...
					EventHandler setPower = (s, ea) => EventManager.AsyncEvent(new LayoutEvent<LayoutBlockDefinitionComponent, ILayoutPower>("set-power", blockDefinition, p));
		...
		private void blockDefinitionQueryDrop(LayoutEvent e) {
			...
			XmlElement element = dragEvent.Data.GetData(typeof(XmlElement)) as XmlElement;
		...
		class SetManualDispatchRouteSourceMenuItem : MenuItem {
			ManualDispatchRouteSetting	manualDispatchRouteSetting;
		...
		class CreateManualDispatchRouteMenuItem : MenuItem {
			ManualDispatchRouteSetting	manualDispatchRouteSetting;
			LayoutBlockDefinitionComponent	blockInfo;
			LocomotiveOrientation		direction = LocomotiveOrientation.Unknown;
		...
		class AddBlockInfoToTripPlanEditorDialog : MenuItem {
			...
			ITripPlanEditorDialog tripPlanEditorDialog;
		...
		private MenuItem getUseSavedTripPlanMenuItem(LayoutBlockDefinitionComponent blockInfo, TrainStateInfo train) {
			Guid		blockID = blockInfo.Block.Id;
		...
		class SaveTrainInCollectionMenuItem : MenuItem {
			...
			protected override void OnClick(EventArgs e) {
				...
				XmlElement					trainInCollectionElement = null;
				TrainInCollectionInfo		trainInCollection = null;

				trainInCollectionElement = collection["Train", train.Name];
		...
		class ExtendTrainMenuItem : MenuItem {
			LayoutBlock			block;
			ExtendableTrainInfo	extendableTrainInfo;
			...
			protected override void OnClick(EventArgs e) {
				...
				if(extendableTrainInfo.blockEdge is LayoutTrackContactComponent) {
					...
					trackContactPassingState.FromBlock = block;
					trackContactPassingState.ToBlock = extendableTrainInfo.Block;
					trackContactPassingState.FromBlockTriggerCount = fromCount;
					trackContactPassingState.ToBlockTriggerCount = toCount;
