...
namespace LayoutManager.Logic {
	...
	public class TrainMotionManager : LayoutModuleBase {
		...
		Dictionary<Guid, SpeedChangeState> trainIDtoSpeedChangeState = new Dictionary<Guid,SpeedChangeState>();
		...
		private void trainSpeedChangeRequest(LayoutEvent e) {
			...
			if(train.Speed != requestedSpeed) {
				...
				if(ramp.RampType == MotionRampType.LocomotiveHardware)
					...
					SpeedChangeState			speedChangeState;
					int							motionTime;

					if(trainIDtoSpeedChangeState.TryGetValue(train.Id, out speedChangeState))
						speedChangeState.Dispose();

					speedChangeState = new SpeedChangeState(train);

					speedChangeState.LogicalToTrainSpeedStepsFactor = logicalToTrainSpeedStepsFactor;
		...
		private void trainSpeedChangeAbort(LayoutEvent e) {
			TrainStateInfo		train = (TrainStateInfo)e.Sender;
			SpeedChangeState speedStateChange;

			if(trainIDtoSpeedChangeState.TryGetValue(train.Id, out speedStateChange))
