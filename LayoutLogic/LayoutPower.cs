	...
	class LayoutPowerManager : LayoutModuleBase {
		...
		private bool checkForReverseLoopsForPowerSection(LayoutPhase phase) {
			...
			foreach(var split in LayoutModel.Components<IModelComponentIsMultiPath>(phase)) {
				...
				while(scanStack.Any()) {
					...
					var splitTrack = edge.Track as IModelComponentIsMultiPath;
			...
			if(!powerConnectors.Any()) {
				...
				if(tracksWithNoPower.Count > 0) {
					...
					var invalidProgrammingBlocks = new LayoutSelection(from blockDefinition in LayoutModel.Components<LayoutBlockDefinitionComponent>(phase) 
													   ...
													   !blockDefinition.Track.GetPowerConnector(blockDefinition.Track.ConnectionPoints[0]).Inlet.ConnectedOutlet.ObtainablePowers.Any(p => p.Type == LayoutPowerType.Programmer) select blockDefinition);
		...
		private void checkPowerSelectors(LayoutEvent e) {
			...
			foreach(var powerSelector in powerSelectorComponents) {
				if(!checkInlet(powerSelector, powerSelector.Inlet1, "Inlet 1", phase))
					e.Info = false;
				if(!checkInlet(powerSelector, powerSelector.Inlet2, "Inlet 2", phase))
					e.Info = false;
			}
		}

		private bool checkInlet(LayoutPowerSelectorComponent powerSelector, ILayoutPowerInlet inlet, string inletName, LayoutPhase phase) {
		...
		private async Task setPower(LayoutEvent e) {
			...
			if(powerConnector.Inlet.ConnectedOutlet.Power.Type != power.Type) {
				...
				await EventManager.AsyncEvent(new LayoutEvent<object, List<SwitchingCommand>>("set-track-components-state", info: switchingCommands));
